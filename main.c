...
// Compute the sum of new Page Rank scores across all nodes using MPI_Allreduce
...
// Compute the sum of differences across all nodes using MPI_Reduce
double total_diff = 0.0;
MPI_Reduce(&diff, &total_diff, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

// Update the Page Rank scores using MPI_Allgather to gather scores from all nodes
MPI_Allgather(&new_page_rank[my_rank], n / comm_size, MPI_DOUBLE, page_rank, n / comm_size, MPI_DOUBLE, MPI_COMM_WORLD);
...
total_time = (double)(end_time_total - start_time_total) / CLOCKS_PER_SEC;
